...
export default function Projects({ projects, tasks, milestones, risks, dependencies, uatSit, golive, vendors, users, role, selected, setSelected }) {
  ...
  // Cosmetic only right now — real enforcement needs Sprint 4's backend
  // permission checks. This just hides buttons for a nicer demo.
  ...
  const trackList = allProjects.filter((p) => (p.track ?? (p.projectType || "IT").toLowerCase()) === track);
  const list = trackList.filter((p) => filterHealth === "All" || p.health === filterHealth);
  const detail = allProjects.find((p) => p.name === selected);
  const itCount = allProjects.filter((p) => (p.track ?? (p.projectType || "IT").toLowerCase()) === "it").length;
  const businessCount = allProjects.filter((p) => (p.track ?? (p.projectType || "IT").toLowerCase()) === "business").length;
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5">
            {["All", "Red", "Amber", "Green", "Unknown"].map((h) => (
              <button key={h} onClick={() => setFilterHealth(h)} className={`px-2.5 py-1 rounded text-[11px] font-medium border ${filterHealth === h ? "bg-active border-accent text-primary" : "border-default text-tertiary"}`}>{h}</button>
            ))}
          </div>
          {canManage && (
            ...
            setExtraProjects((prev) => [
              {
                ...
                domain: project.domain || "—", business: project.business || "—",
                lead: leadName, leadId: project.lead_id, priority: project.priority, status: project.status,
                phase: project.phase || "—", progress: 0, health: "Green",
                delayDays: 0, escalation: "No", blocker: "", nextAction: "—", remarks: "",
              },
              ...prev,
            ]);
            setShowForm(false);
